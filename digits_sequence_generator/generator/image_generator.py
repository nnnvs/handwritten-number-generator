...
class ImageGenerator:
    ...
    def generate_digits_sequence(self):
        ...
        post_processor = post_processing.PostProcessing(self.task_id, self.number, self.spacing_min, self.spacing_max, self.image_width)
        processed_image = post_processor.post_processing(augmented_images)

        return processed_image

    def fetch_random_images(self):
        chosen_images=[]
        logger.info("[Task Id: " + str(self.task_id) + "]. Choosing random images for " + str(self.number) + " to augment")
